  ...
  return (
    <div className={`flex w-full items-center  justify-between px-6 py-4 shadow-md fixed z-10 ${theme === 'light' ? 'bg-white' : 'bg-gray-800'}`}>
      <div className="ml-10 lg:ml-60">
        ...
        <p className={`text-sm ${theme === 'light' ? 'text-gray-500' : 'text-gray-300'}`}>Welcome back, {userLogIn?.name || 'Admin'}</p>
      </div>
      ...
      <div className="flex items-center lg:gap-6">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className={`${theme === 'light' ? 'text-gray-700' : 'text-gray-300'}`}>{userLogIn?.email || 'admin@ihuza.com'}</span>
